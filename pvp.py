...
class BhtcPVP(PVP):
    """
    Pattern-verbalizer pair (PVP) for the BHTC Corpus.
    """
    ...
    VERBALIZER = {
        "Ekonomia": ["Ekonomia"],
        "Euskal_Herria": ["Euskadi"],
        "Euskara": ["Euskara"],
        "Gizartea": ["Gizartea"],
        "Historia": ["Historia"],
        "Ingurumena": ["Ingurumena"],
        "Iritzia": ["Iritzi"],
        "Komunikazioa": ["Komunikazioa"],
        "Kultura": ["Kultura"],
        "Nazioartea": ["Nazioarte"],
        "Politika": ["Politika"],
        "Zientzia": ["Zientzia"]
        }
    ...
    def get_parts(self, example: InputExample):
        ...
        if self.pattern_id == 0:
            ...
            # this corresponds to the pattern [MASK] berriak: a
            return [self.mask, 'berriak:', text_a, text_b], []
        elif self.pattern_id == 2:
            # this corresponds to the pattern [MASK] berriak: a
            return ['[Gaia:', self.mask, ']', text_a, text_b], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))

    def verbalize(self, label) -> List[str]:
        return BhtcPVP.VERBALIZER[label]


# register the PVP for this task with its name
PVPS[BhtcPVP.TASK_NAME] = BhtcPVP
