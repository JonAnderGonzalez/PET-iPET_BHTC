...
class BhtcProcessor(DataProcessor):
    """
    Processor for the Basque BHTC corpus.
    """
    ...
    LABELS = ["Ekonomia", "Euskal_Herria", "Euskara", "Gizartea", "Historia", "Ingurumena", "Iritzia", "Komunikazioa", "Kultura", "Nazioartea", "Politika", "Zientzia"] 

    # Set this to the column of the train/test csv files containing the input's text a
    TEXT_A_COLUMN = 1

    # -1 if there is no text b
    TEXT_B_COLUMN = -1
    ...
    def get_train_examples(self, data_dir: str) -> List[InputExample]:
        ...
        """
        return self._create_examples(os.path.join(data_dir, BhtcProcessor.TRAIN_FILE_NAME), "train")
    ...
    def get_dev_examples(self, data_dir: str) -> List[InputExample]:
        ...
        """
        return self._create_examples(os.path.join(data_dir, BhtcProcessor.DEV_FILE_NAME), "dev")
    ...
    def get_test_examples(self, data_dir) -> List[InputExample]:
        ...
        """
        return self._create_examples(os.path.join(data_dir, BhtcProcessor.TEST_FILE_NAME), "test")
    ...
    def get_unlabeled_examples(self, data_dir) -> List[InputExample]:
        ...
        """
        return self._create_examples(os.path.join(data_dir, BhtcProcessor.UNLABELED_FILE_NAME), "unlabeled")

    def get_labels(self) -> List[str]:
        """This method returns all possible labels for the task."""
        return BhtcProcessor.LABELS
    ...
    def _create_examples(self, path, set_type, max_examples=-1, skip_first=0):
        ...
        with open(path) as f:
            ...
            for idx, row in enumerate(reader):
                guid = "%s-%s" % (set_type, idx)
                label = row[BhtcProcessor.LABEL_COLUMN]
                text_a = row[BhtcProcessor.TEXT_A_COLUMN]
                text_b = row[BhtcProcessor.TEXT_B_COLUMN] if BhtcProcessor.TEXT_B_COLUMN >= 0 else None
                example = InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label)
                examples.append(example)
...
PROCESSORS[BhtcProcessor.TASK_NAME] = BhtcProcessor
#multi token masks
# TASK_HELPERS['basque-topic-classification'] = MultiMaskTaskHelper
